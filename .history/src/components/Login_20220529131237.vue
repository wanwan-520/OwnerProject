<template>
    ...
    <!-- 布置一个子模块 -->
     <div class="login_box">
         ...
         <!-- 登录表单区域 -->
         <!-- 使用el-element组件库 -->
         <!-- 绑定ref属性直接获取DOM元素中内容，使用方式this.$refs['自定义属性名'] -->
  ...
  <el-form-item prop="username">
   ...
   <!-- 表单的重置按钮 -->
   <el-button  round @click='resetLoginForms'>重置</el-button>
  </el-form-item>
  </el-form>
  </div>
    </div>
        ...
        login(){
            ...
            //消息提示框引入
              if(res.meta.status !==200) return this.$message.error('失败');
               ...
               //浏览器调用sessionStorage方法保存token里面的内容
...
.login_box{
    ...
    /* background-color: rgb(20, 198, 97); */
     ...
     background-color: background-image: linear-gradient(to top, #a18cd1 0%, #fbc2eb 100%);;
    border-radius: 2px;
    /* 相对于父元素进行定位 */
    position: absolute;
    left:50%;
